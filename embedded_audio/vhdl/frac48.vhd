...
-- Command Line : fracn09.pl 5e7 48000
-- Date         : Sun Feb 24 21:49:06 2013
...
-- Desired Output Frequency: 48000 Hz.
-- Requested Relative Frequency Error Bounds (+/-) : 1e-07 (0.1 ppm)
--
-- Frequency Results (use_phase_accumulator = FALSE) :
--  Achieved Output Frequency: 48000 Hz.
--  Achieved Relative Frequency Error: 0 (0 ppm)
--  Achieved Frequency Error: 0 Hz.
--
-- Frequency Results (use_phase_accumulator = TRUE) :
--  Achieved Output Frequency: 48000.0046081841 Hz.
--  Achieved Relative Frequency Error: 9.60038353999456e-08 (0.0960038353999456 ppm)
--  Achieved Frequency Error: 0.00460818409919739 Hz.
--
-- Output Jitter Parameters (use_phase_accumulator = FALSE) :
--  The fundamental frequency is 16000 Hz.
--  The amplitude is 1.33333333333348e-08 seconds p-p (minimum_jitter = FALSE).
--  The amplitude is 1.33333333333348e-08 seconds p-p (minimum_jitter = TRUE).
--
-- Output Jitter Parameters (use_phase_accumulator = TRUE) :
--  The fundamental frequency is 0.0116415321826935 Hz.
--  The amplitude is 2e-08 seconds p-p (approx).
--
-- Design Parameters (use_phase_accumulator = FALSE) :
--  Approx 16 flip flops (11 in prescaler, 3 in controller and 2 retimes).
--  The recursive controller uses approx 3 flip flops.
--  The Dual-Modulus Prescaler uses ratios /1041,/1042
--  The Output consists of 1 cycle of 1041 input clocks,
--  and 2 cycles of 1042 input clocks.
--  There are 3 output clocks for every 3125 input clocks.
--
-- Design Parameters (use_phase_accumulator = TRUE) :
--  Approx 34 flip flops (32 in accumulator and 2 retimes)
--  There are 4123169 output clocks for every 4294967296 input clocks.
--
-- Divider summary :
--
...
--
--  34    tbd       9.6e-08   2e-08     use_phase_accumulator
--  37    tbd       9.6e-08   1e-08     use_phase_accumulator improve_duty_cycle
--  16    tbd       0         1.3e-08   use_recursive_controller
--  17    tbd       0         1.3e-08   use_recursive_controller improve_duty_cycle
--  16    tbd       0         1.3e-08   minimum_jitter
--  17    tbd       0         1.3e-08   minimum_jitter improve_duty_cycle
--  16    tbd       0         1.3e-08   (none)
--  17    tbd       0         1.3e-08   (none) improve_duty_cycle
--
-- Warnings:
--  none
--
...
entity frac48 is
    ...
    port (
        ...
        output_50         : out std_logic;  -- 48000 Hz output - approx 50% duty cycle
        output_pulse      : out std_logic   -- 48000 Hz output - high for single clock per cycle
    );
end frac48;

architecture rtl of frac48 is

    -- definitions for prescaler / controller design
    constant n              : positive := 1041; -- prescaler divides by n or n + 1
    constant a              : positive := 1;    -- this many counts of 1041
    constant b              : natural  := 2;    -- this many counts of 1042
    ...
    constant num_bits       : positive := 32;   -- size of phase accumulator
...
--------------------------------------------------------------------------------
-- Prescaler.  Divides by either 1041 or 1042
...
-- and '1' for 2 counts (out of a total of 3 counts).
-- The simple way to do this is to just have modulus_control '0' for
-- all 1 counts, then '1' for 2 counts, but this may result in severe jitter.
...
--          n0=1041     m0=X        a0=1        b0=2        i0=X
--          n1=2        m1=1        a1=X        b1=X        i1=0
--------------------------------------------------------------------------------
recursive_controller : if not use_phase_accumulator and use_recursive_controller generate
    ...
    begin
        if (async_reset = '1') then
            ...
            if (clock_enable = '1') then
                -- Stage 1  stage1_out is low for 1 cycles, and high for 2 cycles.
                -- n1=2 m1=1 a1=X b1=X i1=0
                if (prescaler_out = '1') then
