--------------------------------------------------------------------------------
-- File         : frac8.vhd (machine generated)
-- Contains     : entity frac8 (architecture rtl)
-- Author       : fracn09.pl  (version 0.09)
-- Command Line : fracn09.pl -e frac8 5e7 8000
-- Date         : Mon Feb 25 00:26:47 2013
...
-- Desired Output Frequency: 8000 Hz.
-- Requested Relative Frequency Error Bounds (+/-) : 1e-07 (0.1 ppm)
--
-- Frequency Results (use_phase_accumulator = FALSE) :
--  Achieved Output Frequency: 8000 Hz.
--  Achieved Relative Frequency Error: 0 (0 ppm)
--  Achieved Frequency Error: 0 Hz.
--
-- Frequency Results (use_phase_accumulator = TRUE) :
--  Achieved Output Frequency: 7999.999797903 Hz.
--  Achieved Relative Frequency Error: -2.52621248364449e-08 (-0.0252621248364449 ppm)
--  Achieved Frequency Error: -0.000202096998691559 Hz.
--
...
--  The fundamental frequency is 0.00291038304567337 Hz.
--  The amplitude is 2e-08 seconds p-p (approx).
--
-- Design Parameters (use_phase_accumulator = FALSE) :
--  Approx 16 flip flops (13 in prescaler, 1 in controller and 2 retimes).
--  The recursive controller uses approx 1 flip flop.
--  The Dual-Modulus Prescaler uses ratios /6250,/6251
--  The Output consists of 1 cycle of 6250 input clocks,
--  and 0 cycles of 6251 input clocks.
--  There is 1 output clock for every 6250 input clocks.
--
-- Design Parameters (use_phase_accumulator = TRUE) :
--  Approx 36 flip flops (34 in accumulator and 2 retimes)
--  There are 2748779 output clocks for every 17179869184 input clocks.
--
-- Divider summary :
--
...
--
--  36    tbd       -2.5e-08  2e-08     use_phase_accumulator
--  39    tbd       -2.5e-08  1e-08     use_phase_accumulator improve_duty_cycle
--  16    tbd       0         0         use_recursive_controller
--  17    tbd       0         0         use_recursive_controller improve_duty_cycle
--  16    tbd       0         0         minimum_jitter
--  17    tbd       0         0         minimum_jitter improve_duty_cycle
--  16    tbd       0         0         (none)
--  17    tbd       0         0         (none) improve_duty_cycle
--
-- Warnings:
--* Warning: a fractional-N divider is not needed.
--*   Integer divider 6250 is ok.
--
...
entity frac8 is
    ...
    port (
        ...
        output_50         : out std_logic;  -- 8000 Hz output - approx 50% duty cycle
        output_pulse      : out std_logic   -- 8000 Hz output - high for single clock per cycle
    );
end frac8;

architecture rtl of frac8 is

    -- definitions for prescaler / controller design
    constant n              : positive := 6250; -- prescaler divides by n or n + 1
    constant a              : positive := 1;    -- this many counts of 6250
    constant b              : natural  := 0;    -- this many counts of 6251
    ...
    constant num_bits       : positive := 34;   -- size of phase accumulator
    constant c              : unsigned(num_bits - 1 downto 0) := to_unsigned(2748779, num_bits);
...
--------------------------------------------------------------------------------
-- Prescaler.  Divides by either 6250 or 6251
